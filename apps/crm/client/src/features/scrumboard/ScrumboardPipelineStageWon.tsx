...
function ScrumboardColumnWon({ stageId }: Props) {
  ...
  return (
    <div ref={stageRef} className={`${styles.column} ${styles[`column--won`]}`}>
      <div className={`${styles.column__header} ${styles[`column__header--won`]}`}>
        <div className={styles.headerPanel}>
          <div className={styles.headerDetails}>
            <span className={styles['headerDetails__title--won']}>{stage.title}</span>
      ...
      <div className={styles.column__cards}>
        {dealIdsLexiSorted.map((dealId, i) => {
          return <ScrumboardPipelineDeal key={dealId} dealId={dealId} dealIndex={i} stage={stage} dealStatus="won" />;
        })}
      </div>
    </div>
  );
...
export default memo(ScrumboardColumnWon);
