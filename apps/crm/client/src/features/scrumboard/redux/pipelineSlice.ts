...
const pipelineSlice = createSlice({
  ...
  reducers: {
    ...
    createDeal(state, action: PayloadAction<ICreateDealPayload>) {
      const { columnId, companyTitle, dealTitle, dealStage, dealValue, dealOwner } = action.payload;

      const newTaskId = `task-${Math.floor(Math.random() * 100_000)}`;
      const newTask = {
        id: newTaskId,
        ...
        userImage: UserImage,
        daysElapsed: 27,
        dealTotal: Number(dealValue),
      };

      console.log(dealOwner, dealStage); // Ignored for now.
    ...
    // updateDeal(state, action: PayloadAction<>) {},
